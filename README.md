# 🎮 **UnScramble Me!**

## 📌 Rangkuman Project

        UnScramble Me! adalah sebuah permainan kata sederhana berbasis bahasa C++ yang dijalankan melalui
    terminal dengan memanfaatkan library ncurses. Permainan ini menyajikan sebuah kata yang huruf-hurufnya
    telah diacak, kemudian pemain ditantang untuk menebak dan menyusun kembali kata tersebut menjadi kata
    yang benar.
    
        Kata-kata yang digunakan dalam permainan ini diambil dari istilah-istilah pada mata kuliah
    pemrograman, sehingga permainan ini tidak hanya bersifat hiburan, tetapi juga memiliki nilai edukatif.
    Melalui permainan ini, pemain diharapkan dapat meningkatkan kemampuan dalam mengenali, menyusun, serta
    memahami istilah-istilah pemrograman.
________________________________________
## 🎯 Tujuan Project

    Adapun tujuan dari pembuatan project UnScramble Me! adalah sebagai berikut:
        1. Melatih logika dan ketelitian pemain dalam menyusun kata yang diacak.
        2. Meningkatkan pemahaman terhadap istilah-istilah dasar dalam pemrograman.
        3. Mengimplementasikan konsep dasar bahasa C dalam bentuk aplikasi permainan.
        4. Menerapkan penggunaan library ncurses untuk membangun antarmuka berbasis terminal.
        5. Melatih kerja sama tim melalui pembagian tugas dan penggunaan version control GitHub.
________________________________________
## 🕹️ Alur Permainan

        1. Program menampilkan judul permainan dengan efek animasi.
        2. Sistem memilih satu kata secara acak dari kumpulan kata pemrograman.
        3. Huruf-huruf pada kata tersebut diacak secara otomatis.
        4. Pemain diminta untuk memasukkan jawaban sesuai dengan kata yang benar.
        5. Sistem melakukan pengecekan jawaban dan memberikan skor.
        6. Permainan berlangsung dalam beberapa soal pada satu sesi.
        7. Di akhir permainan, skor akhir ditampilkan dan pemain dapat memilih untuk bermain kembali
           atau keluar.
________________________________________
## 🧠 Fitur Utama

        • Antarmuka interaktif berbasis terminal menggunakan ncurses
        • Animasi teks pada tampilan judul permainan
        • Pengacakan kata secara acak setiap soal
        • Sistem penilaian (skor)
        • Pilihan untuk bermain ulang
        • Penggunaan warna untuk memperjelas tampilan permainan
________________________________________
## 🛠️ Teknologi yang Digunakan

        • Bahasa: C++
        • Library UI: ncurses (antarmuka terminal)
        • Format Data: Penyimpanan berbasis teks di data/stuff.txt
        • Sistem Menu: Pemilihan interaktif berbasis keyboard
________________________________________
## ▶️ Cara Menjalankan Program

        1. Pastikan library ncurses telah terpasang pada sistem.
        2. Lakukan proses kompilasi program dengan perintah:
                " gcc unscramble.cpp -lncurses -o unscramble "
        3. Jalankan program dengan perintah:
                " ./unscramble "
________________________________________
## 📌 Penutup

        Project UnScramble Me! diharapkan dapat menjadi media pembelajaran yang menarik dan interaktif,
    khususnya dalam mengenal sedikit kata-kata pemrograman. Selain itu, project ini juga melatih mahasiswa
    dalam pengetahuan bahasa pemrograman. Kami menyadari masih banyak kekurangan dari project kami ini,
    sehingga kritik dan saran sangat dibutuhkan untuk mengembangkan project kami. Agar bisa lebih
    bermanfaat untuk masa mendatang.
